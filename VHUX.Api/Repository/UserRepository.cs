    ...
    public class UserRepository: IUserRepository
    {
        ...
        public async Task<PaginationSet<UserModel>> UserList(string? full_name, string? username, int page_number, int page_size)
        {
            ...
            if (page_number > 0)
            {
                ...
                response.list = listItem.OrderByDescending(r => r.id).Skip(page_size * (page_number - 1)).Take(page_size).ToList();
            }
            else
            {
                response.list = listItem.OrderByDescending(r => r.id).ToList();
            }
            return response;
        }
